...
export function ShopPage() {
  ...
  return (
    <AppShell active="browse" searchPlaceholder="Search marketplace...">
      ...
      {!slug && <StatePanel action={<a className="button inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border px-5 py-2.5 text-center text-sm font-bold transition disabled:pointer-events-none disabled:opacity-50 [&.full]:w-full button-secondary border-foose-border bg-foose-surface text-foose-text hover:border-accent hover:text-accent" href={withBasePath('/digishops')}>Browse DigiShops</a>} body="This link does not identify a DigiShop." layout="page" title="Shop link is incomplete" tone="unavailable" />}
      ...
      {shop.error && <StatePanel action={<button className="button inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border px-5 py-2.5 text-center text-sm font-bold transition disabled:pointer-events-none disabled:opacity-50 [&.full]:w-full button-secondary border-foose-border bg-foose-surface text-foose-text hover:border-accent hover:text-accent" onClick={shop.refetch} type="button">Try again</button>} body={shop.error} layout="page" title={shop.errorMeta?.status === 404 ? 'This DigiShop is unavailable' : 'DigiShop could not load'} tone={shop.errorMeta?.status === 403 ? 'permission' : shop.errorMeta?.status === 404 ? 'unavailable' : 'error'} />}
      {shopData && (
        <div className="grid gap-6 lg:h-[calc(100dvh-9rem)] lg:min-h-0 lg:grid-cols-[minmax(0,1fr)_320px] lg:overflow-hidden xl:grid-cols-[minmax(0,1fr)_360px]">
          <section className="order-2 min-w-0 lg:order-1 lg:min-h-0 lg:overflow-y-auto lg:pr-2 lg:[scrollbar-width:none] lg:[&::-webkit-scrollbar]:hidden">
            ...
            {listings.error && !filteredListings.length && <StatePanel action={<button className="button inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border px-5 py-2.5 text-center text-sm font-bold transition disabled:pointer-events-none disabled:opacity-50 [&.full]:w-full button-secondary border-foose-border bg-foose-surface text-foose-text hover:border-accent hover:text-accent" onClick={listings.refetch} type="button">Retry listings</button>} body={listings.error} layout="section" title="Shop listings could not load" tone="error" />}
